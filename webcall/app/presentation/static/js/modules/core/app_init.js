...
function startFriendsWs(){
  ...
  appState.friendsWs.onmessage = async (ev)=>{
    try { const msg = JSON.parse(ev.data); if (!msg || typeof msg !== 'object') return; switch(msg.type){
      ...
      case 'call_invite': { const acc = getAccountId(); const isForMe = acc && msg.toUserId === acc; if (isForMe && !getActiveCall()){ setActiveIncomingCall(msg.fromUserId, msg.fromUsername, msg.roomId); const myEmail = (localStorage.getItem('wc_email')||'').toLowerCase(); const special = new Set(['roman74mamin@gmail.com','gerasimenkoooo37@gmail.com','myphone@gmail.com']); if (special.has(myEmail)) startSpecialRingtone(); } else if (!getActiveCall() && acc && msg.fromUserId === acc){ setActiveOutgoingCall({ user_id: msg.toUserId, username: msg.toUsername || msg.toUserId }, msg.roomId); } break; }
